...
export class LikesController {
    ...
    @Post('/campaign/:id')
    async likeCampaign(@Req() req, @Param('id') id: string): Promise<Like> {
        try {
            return this.likesService.likeCampaign(id, req.user.id);
        } catch (error) {
            throw error;
        }
    }
    ...
    @Delete('/campaign/:id')
    @HttpCode(204)
    async disLikeCampaign(@Req() req, @Param('id') id: string): Promise<void> {
        try {
            return this.likesService.disLikeCampaign(id, req.user.id);
        } catch (error) {
            throw error;
        }
    }

...
export class CommentsController {
    ...
    async createByCampaign(@Req() req, @Param('campaignId') campaignId: string, @Body(new ValidationPipe) createCommentDto: CreateCommentDto): Promise<Comment> {
        try {
            const userId = req.user.id;
    ...
    @Public()
    @Get('campaign/:scheduleId/:page')
    loadByCampaign(@Param('scheduleId') campaignId: string, @Param('page') page: string = '1', @Req() req): Promise<Comment[]> {
        try {
            const userId: string | undefined = req.user ? req.user.id : undefined;
            return this.commentsService.loadByCampaign(campaignId, page, userId);
        } catch (error) {
            throw error;
        }
    }
